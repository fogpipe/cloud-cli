...
//
// before names the last release a previous page showed, and reads the ones
// older than it — the changelog only grows, so no limit is a lasting answer to
// "show me the rest" (fogpipe/cloud-workspace#1054). The answer says whether
// more remain.
//
...
func (c *Client) Changelog(ctx context.Context, limit int, before string) (*Changelog, error) {
	q := url.Values{}
	if limit > 0 {
...
//
// A load window asks for each app's measured use over it (AppStatus.Load);
// the zero query leaves the document as it is, so a watcher never pays for a
// metrics read it did not ask for.
func (c *Client) ProjectStatus(ctx context.Context, projectID, ifNoneMatch string, load LoadQuery) (*ProjectStatus, string, error) {
	path := "/api/v1/projects/" + projectID + "/status"
	if load.Window != "" {
		q := url.Values{}
		q.Set("load", load.Window)
