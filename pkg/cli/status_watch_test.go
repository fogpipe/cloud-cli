...
func TestWatchPollRecoversWhenCredentialsBecomeValid(t *testing.T) {
	...
	_, _, err := client.New(srv.URL, "stale-token").ProjectStatus(t.Context(), "p1", "", client.LoadQuery{})
	require.Error(t, err)
	...
	valid.Store(true)
	status, _, err := client.New(srv.URL, "fresh-token").ProjectStatus(t.Context(), "p1", "", client.LoadQuery{})
